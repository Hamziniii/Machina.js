...
export class Machina {
     ...
     */
    evaluateMsg(msg: Message): /** Value is the list of functions, reason is the reason why it works or fails, extra is just some extra info for debugging */MachinaResponse<MachinaFunction | MachinaFunction[]>{ // TODO custom prefixes per server, custom verification
        if(msg.author.username == this.client.user.username) return {value: null, reason: "msg author is the same as bot"}
        if(!msg.content.includes(this.PREFIX)) return {value: null, reason: "msg does not include the set prefix"}
        if(!msg.content.startsWith(this.PREFIX)) return {value: null, reason: "msg does not start with correct prefix"}
