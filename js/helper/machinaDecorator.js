...
import { checkArgsAgainstCriteria, convertArgType, exists, arrify, sameObj } from "./machinaUtility";
 ...
 */
export const machinaDecoratorInfo = (info) => 
/**
...
(target, propertyKey, f) => {
    ...
    target[propertyKey] = async (...args) => {
        const [Bot, msg] = args;
        let c = Machina.subCommandMiddleware(msg === null || msg === void 0 ? void 0 : msg.content, info.separator, 1);
        ...
        if (info.strictArgs) // When it errors out
            if (!(results === null || results === void 0 ? void 0 : results.value) || !exists(results)) {
 ...
 */
export const machinaDecoratorClassName = (target, propertyKey) => {
